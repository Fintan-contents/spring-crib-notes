    ...
    stage('alfort push(branch)') {
      ...
      steps {
        withCredentials(
          ...
          sh("git branch temp_branch_${env.BUILD_NUMBER}")
          sh('git push https://${GIT_USERNAME}:${GIT_PASSWORD}@alfort.adc-tis.com/gitbucket/git/keel/spring-crib-notes.git temp_branch_' + env.BUILD_NUMBER + ':${BRANCH_NAME}')
          sh("git branch -D temp_branch_${env.BUILD_NUMBER}")
        }
      }
      post {
        failure {
